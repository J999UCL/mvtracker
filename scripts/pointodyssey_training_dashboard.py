...
function fitLossScale(chart){const values=chart.data.datasets.flatMap((dataset,index)=>{if(dataset.isRawPoints||!chart.isDatasetVisible(index))return[];const raw=chart.data.datasets[index-1];return(raw&&raw.isRawPoints?raw.data:dataset.data).map(point=>Number(point.y)).filter(Number.isFinite);});if(!values.length){delete chart.options.scales.y.min;delete chart.options.scales.y.max;return;}values.sort((a,b)=>a-b);const quantile=p=>values[Math.min(values.length-1,Math.floor((values.length-1)*p))];const minimum=quantile(.01),maximum=quantile(.99),range=maximum-minimum||Math.max(Math.abs(maximum)*.1,1e-6),padding=range*.08;chart.options.scales.y.min=Math.max(0,minimum-padding);chart.options.scales.y.max=maximum+padding;}
