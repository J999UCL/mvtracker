...
function options(xTitle,yTitle,extra={}){
  return {responsive:true,maintainAspectRatio:false,animation:reduced?false:{duration:180},interaction:{mode:'index',intersect:false},plugins:{legend:{display:extra.legend!==false,position:'bottom',...(extra.legendOnClick?{onClick:extra.legendOnClick}:{}),labels:{color:palette.text,usePointStyle:true,boxWidth:8,filter:(item,data)=>data.datasets[item.datasetIndex].showInLegend!==false}},tooltip:{enabled:true,backgroundColor:palette.panel,titleColor:palette.text,bodyColor:palette.text,borderColor:palette.border,borderWidth:1}},scales:{x:{type:extra.xType||'linear',min:xTitle==='Optimizer step'?MIN_OPTIMIZER_STEP:undefined,grid:{color:palette.border},ticks:{color:palette.muted,maxTicksLimit:10},title:{display:true,text:xTitle,color:palette.muted}},y:{min:extra.min,max:extra.max,grid:{color:palette.border},ticks:{color:palette.muted,callback:extra.tickCallback||compactNumber},title:{display:true,text:yTitle,color:palette.muted}},...(extra.scales||{})}};
}
function fitLossScale(chart){const referenceValues=[],displayedValues=[];chart.data.datasets.forEach((dataset,index)=>{if(dataset.isRawPoints||!chart.isDatasetVisible(index))return;const raw=chart.data.datasets[index-1];const input=(raw&&raw.isRawPoints?raw.data:dataset.data).map(point=>Number(point.y)).filter(Number.isFinite);displayedValues.push(...dataset.data.map(point=>Number(point.y)).filter(Number.isFinite));let weighted=0,weightSum=0;const reference=input.map(value=>{weighted=.9*weighted+.1*value;weightSum=.9*weightSum+.1;return weighted/weightSum;});referenceValues.push(...reference.slice(-100));});if(!referenceValues.length){delete chart.options.scales.y.min;delete chart.options.scales.y.max;return;}referenceValues.sort((a,b)=>a-b);const quantile=p=>referenceValues[Math.min(referenceValues.length-1,Math.floor((referenceValues.length-1)*p))];const minimum=Math.min(quantile(.01),...displayedValues),maximum=Math.max(quantile(.99),...displayedValues),range=maximum-minimum||Math.max(Math.abs(maximum)*.1,1e-6),padding=range*.08;chart.options.scales.y.min=Math.max(0,minimum-padding);chart.options.scales.y.max=maximum+padding;}
