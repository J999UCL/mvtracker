...
function fitLossScale(chart){const visibleValues=[];chart.data.datasets.forEach((dataset,index)=>{if(dataset.isRawPoints||!chart.isDatasetVisible(index))return;visibleValues.push(...dataset.data.filter(point=>Number(point.x)>=MIN_OPTIMIZER_STEP).map(point=>Number(point.y)).filter(Number.isFinite));});if(!visibleValues.length){delete chart.options.scales.y.min;delete chart.options.scales.y.max;return;}const minimum=Math.min(...visibleValues),maximum=Math.max(...visibleValues),range=maximum-minimum||Math.max(Math.abs(maximum)*.1,1e-6),padding=range*.08;chart.options.scales.y.min=Math.max(0,minimum-padding);chart.options.scales.y.max=maximum+padding;}
