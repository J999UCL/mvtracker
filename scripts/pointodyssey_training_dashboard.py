...
function fitLossScale(chart){const visible=chart.data.datasets.filter((dataset,index)=>chart.isDatasetVisible(index)&&!dataset.isRawPoints);const values=visible.flatMap(dataset=>dataset.data.map(point=>Number(point.y)).filter(Number.isFinite));if(!values.length){delete chart.options.scales.y.min;delete chart.options.scales.y.max;return;}const minimum=Math.min(...values),maximum=Math.max(...values),range=maximum-minimum||Math.max(Math.abs(maximum)*.1,1e-6),padding=range*.08;chart.options.scales.y.min=Math.max(0,minimum-padding);chart.options.scales.y.max=maximum+padding;}
